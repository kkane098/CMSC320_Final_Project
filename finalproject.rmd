...
```{r models}
library(broom)
model1 <- lm(log_price~bedrooms+bathrooms, data = log_price_dat %>% filter(price > 0))
model1_coeffs <- model1 %>% tidy()
model1_stats <- model1 %>% glance()

model1_stats

model2 <- lm(log_price~bedrooms+bathrooms+beds+accommodates+room_type+property_type+neighbourhood+has_ac+has_tv+has_washer+has_dryer+has_internet+has_kitchen+has_heating+pets_allowed+review_scores_rating, data = log_price_dat %>% filter(price > 0))
...
model3 <- lm(log_price~bedrooms+bathrooms+accommodates+room_type+property_type+neighbourhood+has_ac+has_tv+pets_allowed+review_scores_rating, data = log_price_dat %>% filter(price > 0))
...
model3_coeffs %>% mutate(multiplier = 10^estimate)
```
